CREATE OR REPLACE PROCEDURE HITDB2.p_DDL_Table (
	IN v_ObjectName VARCHAR(200), 
	IN v_Parameters VARCHAR(100) DEFAULT '')
SPECIFIC HITDB2.p_DDL_Table
RESULT SETS 1
MODIFIES SQL DATA
NOT DETERMINISTIC
...
----------------------------------------------------------------------------------------------------
/*
CALL HITDB2.p_DDL_Table('HITDB2.p_DDL_Table','[UPPER][TRIM][GCR][WSR][CBR]');
CALL HITDB2.p_DDL_Table('HITDB2.p_DDL_Table','[UPPER][WSR]');
SELECT 
'CALL HITDB2.p_DDL_Table(''' || TRIM(PROCSCHEMA) || '.' || TRIM(PROCNAME) || ''',''[UPPER]'');',
'db2 "CALL HITDB2.p_DDL_Table(''' || TRIM(PROCSCHEMA) || '.' || TRIM(PROCNAME) || ''',''[UPPER]'');" -r /LOG/Procedures/' || TRIM(PROCSCHEMA) || '.' || TRIM(PROCNAME) || '.sql'
...
SELECT COL.*
    , COL.TYPENAME 
	|| CASE WHEN COL.TYPENAME IN ('CHARACTER','VARCHAR') THEN '(' || COl.LENGTH ||')' ELSE '' END
	|| CASE WHEN COL.TYPENAME IN ('DECIMAL') THEN '(' || COl.LENGTH ||','|| COL.SCALE || ')' ELSE '' END AS COLTYPE
	
FROM SYSCAT.TABLES TBL
INNER JOIN SYSCAT.COLUMNS COL 
	ON COl.TABSCHEMA = TBL.TABSCHEMA AND COL.TABNAME = TBL.TABNAME 
WHERE 1=1
	AND TBL.OWNERTYPE = 'U' AND TBL.TABSCHEMA NOT IN ('TEMP_DB','SQLTRACE')
	AND TBL.TABNAME = 'BARCODE'
